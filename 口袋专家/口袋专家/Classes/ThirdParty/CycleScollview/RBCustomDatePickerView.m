...
- (void)setTimeBroadcastView
{
    ...
    self.seleceTime = [dateFormatter stringFromDate:nowDate];
    
    //时间是否合法,默认合法
    self.timeIsOk = YES;
    self.backView = [[UIView alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    _backView.backgroundColor = [UIColor clearColor];
//    self.nowPickerShowTimeLabel = [[UILabel alloc] initWithFrame:CGRectMake((Width-278.5)/2+20, 165, 300, 18)];
//    [_nowPickerShowTimeLabel setBackgroundColor:[UIColor redColor]];
//    [_nowPickerShowTimeLabel setFont:[UIFont systemFontOfSize:16.0]];
//    [_nowPickerShowTimeLabel setTextColor:RGBA(51, 51, 51, 1)];
...
//    _nowPickerShowTimeLabel.textColor = [UIColor clearColor];
//    _nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%d月%d日 %@ %@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)]];
//    [self addSubview:_nowPickerShowTimeLabel];
//    
    timeBroadcastView = [[UIView alloc] initWithFrame:CGRectMake((Width-278.5)/2, 230, 258.5, 190.0)];
...
//    [self setSecondScrollView];
    selectTimeIsNotLegalLabel = [[UILabel alloc] initWithFrame:CGRectMake(40, 90+339.5, 278.5, 15)];
...
//    UILabel *secondLabel = [[(UILabel*)[[secondScrollView subviews] objectAtIndex:0] subviews] objectAtIndex:3];
    ...
    NSInteger minuteInt = minuteLabel.tag - 1;
//    NSInteger secondInt = secondLabel.tag - 1;
//    NSString *dateString = [NSString stringWithFormat:@"%ld%02ld%02ld%02ld%02ld",yearInt,monthInt,dayInt,hourInt,minuteInt];
//    NSString *weekString = [self fromDateToWeek:dateString];
//    _nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d年%d月%d日 %@ %02d:%02d",yearInt,monthInt,dayInt,weekString,hourInt,minuteInt];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm"];
    ...
    NSDate *nowDate = [NSDate date];
    self.seleceTime = [dateFormatter stringFromDate:selectDate];
